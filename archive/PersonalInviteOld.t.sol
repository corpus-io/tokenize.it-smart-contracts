...
import "../../contracts/archive/PersonalInviteOld.sol";
import "../FakePaymentToken.sol";



contract PersonalInviteOldOldTest is Test {
    AllowList list;
    CorpusToken token;
    ...
    function setUp() public {
        ...
        invite = new PersonalInviteOld(trustedForwarder, payable(buyer), payable(receiver), 1, 1000000000000000, price, block.timestamp + 1 days, currency, MintableERC20(address(token)));
    ...
    function testConstructor() public {
        PersonalInviteOld inviteLocal;
        inviteLocal = new PersonalInviteOld(trustedForwarder, payable(buyer), payable(receiver), 1, 1000, 10, block.timestamp + 1 days, currency, MintableERC20(address(token)));
    ...
    */
    function testDealHappyCaseVaryDecimals() public {
        ...
        for (uint8 paymentTokenDecimals=1; paymentTokenDecimals<maxDecimals; paymentTokenDecimals++) {
            ...
            */
            ...
            invite = new PersonalInviteOld(trustedForwarder, payable(buyer), payable(receiver), 1, maxMintAmount, _price, block.timestamp + 1 days, paymentToken, MintableERC20(address(token)));
    ...
    function testFailOverflow() public {
        ...
        currency = new CorpusToken(admin, list, 0x0, "CURRENCY", "CUR");
        vm.prank(owner);
        invite = new PersonalInviteOld(trustedForwarder, payable(buyer), payable(receiver), 1, MAX_INT, 100, block.timestamp + 1 days, currency, MintableERC20(address(token)));
    ...
    function testEzCodeUseCase() public {
        ...
        currency = new CorpusToken(admin, list, 0x0, "CURRENCY", "CUR");
        vm.prank(owner);
        invite = new PersonalInviteOld(trustedForwarder, payable(buyer), payable(receiver), 10000000000000000000, 200000000000000000000, 2, block.timestamp + 1 days, currency, MintableERC20(address(token)));
  ...
  //  ["0x9be15eeadcE10d16aee7eF765f55c5BEDb410204","0x6aEe7ebe278bBd044Ae837cA82E84b210620Cad1","10000000000000000000","200000000000000000000",2e12,1654940279453,"0x07865c6E87B9F70255377e024ace6630C1Eaa37F","0x3fe4799d41cb26e6bc1aa113e31c24ac492ec72b"]
        ...
        currency = new CorpusToken(admin, list, 0x0, "CURRENCY", "CUR");
        vm.prank(owner);
        invite = new PersonalInviteOld(trustedForwarder, payable(buyer), payable(receiver), 10000000000000000000, 200000000000000000000, 2000000000000, block.timestamp + 1 days, currency, MintableERC20(address(token)));
